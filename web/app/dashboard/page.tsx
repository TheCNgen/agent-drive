      ...
      <main className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex border-2 border-black bg-white mb-6">
          <button
            ...
            className={`flex-1 px-6 py-3 font-freeman border-r-2 border-black ${activeTab === 'files' ? 'bg-[#D0B4FF]' : 'bg-white hover:bg-gray-50'}`}
          >
            Files
          </button>
          <button
            onClick={() => handleTabChange('agents')}
            className={`flex-1 px-6 py-3 font-freeman border-r-2 border-black ${activeTab === 'agents' ? 'bg-[#D0B4FF]' : 'bg-white hover:bg-gray-50'}`}
          >
            Agents
          </button>
          <button
            onClick={() => handleTabChange('audit')}
            className={`flex-1 px-6 py-3 font-freeman border-r-2 border-black ${activeTab === 'audit' ? 'bg-[#D0B4FF]' : 'bg-white hover:bg-gray-50'}`}
          >
            Audit
          </button>
          <button
            onClick={() => handleTabChange('settings')}
            className={`flex-1 px-6 py-3 font-freeman ${activeTab === 'settings' ? 'bg-[#D0B4FF]' : 'bg-white hover:bg-gray-50'}`}
          >
            Settings
          </button>
        </div>
