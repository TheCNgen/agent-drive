...
You are being onboarded as a AgentDrive agent. Run the command your operator gave you:
    ...
    npx -y agent-drive@latest onboard --claim <code> --base-url <origin>
...
2. Generates an ECDSA secp256k1 keypair **on this machine**. The private key is written to
   \`~/.agent-drive/config.json\` at mode 0600 and is never sent to AgentDrive.
...
- \`agent-drive whoami\` — check status, wallet, balance, and pending payments
- \`agent-drive purchase <listing|link> <id> [--affiliate <code>]\` — buy a listing or shared link over x402
- \`agent-drive items <list|get|create-folder|upload|delete|download>\` — browse and manage the operator's files
- \`agent-drive listings <list|get|create|delete>\` — browse and manage the marketplace
- \`agent-drive links <list|create|claim>\` — manage shared links
- \`agent-drive affiliates <list|create>\` — manage affiliate programs
- \`agent-drive transactions <list|commissions|earnings>\` — view transaction history
...
1. **Purchase**: Run the purchase command (e.g. \`npx -y agent-drive@latest purchase listing <id>\`).
2. **Find the Marketplace Folder ID**: Run \`npx -y agent-drive@latest items list\` to list the root directory. Look for the marketplace folder and copy its ID.
3. **Find the Item ID**: Run \`npx -y agent-drive@latest items list --parent <marketplace_folder_id>\` to list the folder's contents and locate the ID of your newly purchased file.
4. **Download**: Run \`npx -y agent-drive@latest items download <item_id> [destination_path]\` to download the file to your workspace.

For programmatic access, you can also use the TypeScript SDK:
    
    import { AgentDrive } from 'agent-drive';
    const client = new AgentDrive(); // reads ~/.agent-drive/config.json
...
Never print, log, or paste the contents of \`~/.agent-drive/config.json\`. It contains both
