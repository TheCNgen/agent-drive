...
export async function POST(request: NextRequest) {
  try {
    ...
    const { listingId, sharedLinkId, affiliateUserId, commissionRate } = await request.json();
    ...
    if (listingId) {
      ...
      if (!contentItem || contentItem.seller.toString() !== session.user.id) {
      ...
      if (!contentItem || contentItem.owner.toString() !== session.user.id) {
        return NextResponse.json({ error: 'Shared link not found or unauthorized' }, { status: 404 });
      }
    }
